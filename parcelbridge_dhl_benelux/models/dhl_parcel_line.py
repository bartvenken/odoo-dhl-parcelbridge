...
class DhlParcelLine(models.Model):
    ...
    def _check_envelope_nl_only(self):
        # This mirrors the carrier-level rule in
        # delivery_carrier.DHL_PARCEL_TYPE_COUNTRIES ("ENVELOPE": ["NL"]).
        # We keep the check duplicated here because a MIX carrier has no
        # fixed parcel_type at the carrier level, so the carrier-side
        # constraint cannot catch a per-line ENVELOPE choice. Keep both
        # in sync if DHL ever extends ENVELOPE beyond NL.
        for line in self:
