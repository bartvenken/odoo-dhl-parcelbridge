...
**Why do some webshop customers not see any DHL option in checkout?**
The module filters DHL carriers in webshop checkout by the recipient's
private-vs-business status, on top of Odoo's standard filters (published,
website, country, price-availability). The filter mirrors the backend Add
Shipping wizard so the same set of carriers is offered everywhere.

Concretely, a carrier built around a **consumer-only** parcel type (Envelope
50 to 500 g, Mailbox parcel) is hidden from business customers, and a
carrier built around a **business-only** parcel type (Pallet up to 1000 kg)
is hidden from private customers. The three "Parcel up to X kg" types
(10/20/31) have no recipient restriction and show for both.

If your customers report "no DHL option appears", check whether you have at
least one published DHL carrier that fits the recipient kind they are. The
safest setup for a webshop serving both audiences is one carrier with a
"Parcel up to X kg" type (no recipient restriction); add Envelope / Mailbox
/ Pallet variants on top of that if you want a richer offering, knowing
they auto-hide for the audience they do not fit.
...
**Why can't my shipping manager see the DHL API credentials?**
As of module version 17.0.0.8.8 the three credential fields (User ID, API
...
Non-members can still create, edit and use DHL Parcel carriers - they
just don't see the credential fields on the form and cannot read them
via XML-RPC/ORM. All internal reads by the module itself use `sudo()`,
so day-to-day operations (validating deliveries, printing labels, Test
Connection triggered by an admin who did the setup) work independently
of the operator's group membership.

**Which destinations does the module support, and how is the DHL product picked?**
The module supports outbound from BE / NL / LU to 31 European
destinations. The Countries dropdown on the carrier is filtered to
that list (and further narrowed by parcel type: Envelope = NL only,
Mailbox parcel = BE + NL, other parcels + Pallet = all 31).

For each shipment the module resolves the correct DHL product from
the destination country and the recipient type (business /
consumer):

- Benelux (BE / NL / LU): blank product code; DHL resolves DFY /
  Europlus / Parcel Connect automatically at their side.
- Non-Benelux EU: **CON** (Parcel Connect) for private recipients,
  **EPL-INT** (Europlus International) for business recipients.
- Non-EU (post-Brexit UK, Switzerland, Norway, ...): **CON2C**
  (Parcel Connect 2C) for private, **EPL-INT** for business. A bulk
  customs declaration is added to the request automatically.
- Pallets (Pallet up to 1000 kg): **EPL-PAL** (Europlus Pallet).

**How do I know which DHL products are on my contract?**
Use the **Verify Contract** button in the *Contract check* section
of the DHL Parcel tab on the carrier. It queries DHL's `/products`
endpoint for every enabled destination country + relevant recipient
type, and reports whether the product this carrier would use is
actually active on your DHL contract. Countries flagged NOT IN
CONTRACT will fail at label creation; ask DHL to activate the
missing product, or uncheck those countries on the carrier. It is a
read-only call - nothing is shipped and nothing appears in the DHL
portal.
